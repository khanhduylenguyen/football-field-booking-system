...
import {
  Plus, Edit, Trash2, Search, Tag, Calendar, Image as ImageIcon
} from 'lucide-react';
...
export const OwnerPromotions = () => {
  ...
  const fetchPromotions = async () => {
    try {
      ...
      const response = await fetch(`/api/owner/promotions?${params.toString()}`, {
        headers: token ? {
  ...
  const handleCreate = async () => {
    try {
      const token = localStorage.getItem('auth_token') || sessionStorage.getItem('auth_token');
      const response = await fetch('/api/owner/promotions', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleUpdate = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token') || sessionStorage.getItem('auth_token');
      const response = await fetch(`/api/owner/promotions/${currentPromotion.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">Quản lý tin tức và khuyến mãi của bạn</p>
      </div>
